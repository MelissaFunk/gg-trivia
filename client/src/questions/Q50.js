...
function Q50({ currentUser }) {
  ...
  return(
    <div className="q-div">
      <div className="status-bar-50">
        <h4 className="q-status">Question: 50/50</h4>
      </div>
      ...
      {response}
      {response ? <button className="next-q" onClick={nextQuestion}>Finish</button> : <button className="submit-btn" onClick={handleSubmission}>Submit Answer</button>}
    </div>
  )
}
